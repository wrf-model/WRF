...
while getopts c:r:b:f:n:d:p:k:s:i:e:h opt; do
  case $opt in
    c)
      coreDir="$OPTARG"
    ;;
...
# And prep so other values can be derived
ln -sf $coreDir/* $runDir/

eval "binFirst=\$( realpath \"$runDir/$binFirst\" )"
