...
#	We can choose to do hwrf tests.
...
#( cd WRFV3/test/nmm_real ; ln -s $thedatanmm/wrf_real* . ; cp $thedatanmm/namelist.input.regtest . )
 ...
 ( cd WRFV3/test/nmm_real ; ln -s $thedatanmm/wrfinput_hwrf* . ;  ln -s $thedatanmm/nest1* . ; \
   ln -s $thedatanmm/wrfbdy_hwrf* . ; cp  $thedatanmm/namelist.input.hwrf . ;   )
...
if ( $HWRF == TRUE ) then
	echo "Running HWRF specific NMM-core only" >>! ${DEF_DIR}/wrftest.output
	echo " " >>! ${DEF_DIR}/wrftest.output
endif
if ( $GENERATE_BASELINE != FALSE ) then
...
#DAVE###################################################
		...
		else if ( $core == nmm_real && ( $HWRF == FALSE ) )  then
#DAVE###################################################
...
#DAVE###################################################
		...
		else if ( $core == nmm_real && ( $HWRF == TRUE ) )  then
#DAVE###################################################
echo doing hwrf-nmm pre 
banner 19
#set ans = "$<"
#DAVE###################################################

                        set compopt = $COMPOPTS[3]   # ! parallel only
                        set filetag = 2008-09-09_00:00:00
                        set phys_option=1
                        pushd test/$core
...
#DAVE###################################################
			...
			cp ${CUR_DIR}/io_format io_format
			sed -e '/^ io_form_history /,/^ io_form_boundary/d' -e '/^ restart_interval/r ./io_format' \
			    namelist.input.hwrf >! namelist.input.temp
			...
			sed -e 's/^ run_hours *= *[0-9]*/ run_hours = 0 /' \
			    ...
			    namelist.input.temp >! namelist.input
                        ...
                        ln -s wrfinput_hwrf_d01.${filetag} wrfinput_d01 ;
                        ln -s wrfbdy_hwrf_d01.${filetag} wrfbdy_d01 ;
#DAVE###################################################
echo did cp of namelist hwrf
ls -ls namelist.input
cat namelist.input
banner 19b
#set ans = "$<"
