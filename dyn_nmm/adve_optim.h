...
#ifdef CFLOPT
!***  MASS POINTS
        CF_N = ABS(PETDT(I,K,J)*DT*HBM2(I,J))
        CF_D = ABS(0.5*(DPDE(I,K+1,J0_00)+DPDE(I,K,J0_00)))*CFL_MAX
        IF ( CF_N .GT. CF_D ) THEN
#else
!
!***  MASS POINTS
!
...
!
#ifdef CFLOPT
        CF_N = ABS((PETDT(I+IVW(J),K,J)+PETDT(I+IVE(J),K,J))*DT*VBM2(I,J))
        CF_D = ABS(((DPDE(I+IVW(J),K+1,J0_00)+DPDE(I+IVE(J),K+1,J0_00)         &
     &              +DPDE(I+IVW(J),K  ,J0_00)+DPDE(I+IVE(J),K  ,J0_00))*0.5))*CFL_MAX
        IF ( CF_N .GT. CF_D ) THEN
#else
...
!
#ifdef CFLOPT
        CF_N = ABS((PETDT(I,K,J-1)+PETDT(I,K,J+1))*DT*VBM2(I,J))
        CF_D = ABS(((DPDE(I,K+1,J0_M1)+DPDE(I,K+1,J0_P1)                       &
     &       +DPDE(I,K  ,J0_M1)+DPDE(I,K  ,J0_P1))*0.5))*CFL_MAX
        IF ( CF_N .GT. CF_D ) THEN
#else
