...
RSL_LITE_TO_CHILD_INFO ( msize_p,
  ...
  int_p
     cips_p, cipe_p, cjps_p, cjpe_p
    ,nids_p, nide_p, njds_p, njde_p
...
/* first time */
  if ( stage == NULL ) {
    cd_i = *cips_p ;
    cd_j = *cjps_p ;
    stage = RSL_MALLOC( stage_point_t , (*nide_p-*nids_p+1) * (*njde_p-*njds_p+1) ) ;  /* big enough to hold nest points */
  }
  while ( cd_j <= *cjpe_p ) {
    while ( cd_i <= *cipe_p ) {
      if ( ( *cjps_p <= cd_j && cd_j <= *cjpe_p && *jcoord_p <= cd_j && cd_j <= *jcoord_p+*jdim_cd_p-1 ) &&
           ( *cips_p <= cd_i && cd_i <= *cipe_p && *icoord_p <= cd_i && cd_i <= *icoord_p+*idim_cd_p-1 ) ) {
        *ig_p = cd_i ;
        *jg_p = cd_j ;
        *nig_p = *ig_p - *icoord_p + 1 ;
        *njg_p = *jg_p - *jcoord_p + 1 ;
        cd_i++ ;
        *retval_p = 1 ;
        return ;
      }
